...
class _AddQuestionScreenState
  ...
  final _packTitleController = TextEditingController();
  ...
  int _selectedCategoryId = AppCategories.all.first.id;

  final List<_DraftQuestion> _draftQuestions = <_DraftQuestion>[];
  ...
  void dispose() {
    ...
    for (final draft in _draftQuestions) {
      draft.dispose();
    }
  ...
  void _addHashtag() {
    if (_hashtags.length >= maxHashtags) {
      _showMessage('يمكنك إضافة 5 هاشتاقات كحد أقصى.');
      return;
    }

    final hashtag = _normalizeHashtag(
    ...
    if (hashtag.length > maxHashtagLength) {
      _showMessage('الهاشتاق طويل جدًا.');
      return;
    }

    if (hashtag.contains(' ')) {
      _showMessage('الهاشتاق لا يمكن أن يحتوي على مسافات.');
      return;
    }

    if (hashtag.contains('#')) {
      _showMessage('اكتب هاشتاقًا واحدًا فقط.');
      return;
    }

    if (_hashtags.contains(hashtag)) {
      _showMessage('هذا الهاشتاق مضاف بالفعل.');
      return;
    }
  ...
  void _addQuestion() {
    if (_draftQuestions.length >= maxQuestions) {
      _showMessage('يمكنك إضافة 5 أسئلة كحد أقصى.');
      return;
    }
  ...
  void _removeQuestion(int index) {
    ...
    final draft = _draftQuestions.removeAt(index);
    draft.dispose();
  ...
  Future<void> _publish() async {
    ...
    final packId = _newId('pack');

    final createdQuestions = <Question>[];

    for (var index = 0; index < _draftQuestions.length; index++) {
      final draft = _draftQuestions[index];

      if (draft.type == QuestionType.quiz &&
      ...
      final optionTexts = [
        draft.option1Controller.text.trim(),
        draft.option2Controller.text.trim(),
        draft.option3Controller.text.trim(),
      ];

      final options = List<QuestionOption>.generate(
        optionTexts.length,
        (optionIndex) => QuestionOption(
          id: _newId('option'),
      ...
      final correctOptionId =
          draft.type == QuestionType.quiz
              ? options[draft.correctOptionIndex!].id
              : null;
    ...
    final pack = QuestionPack(
      ...
      title: _packTitleController.text.trim().isEmpty
          ? 'مجموعة ${currentUser.displayName}'
          : _packTitleController.text.trim(),
    ...
    _showMessage('تم نشر المجموعة بنجاح.', success: true);
  ...
  Widget _questionEditor({required int index}) {
    final draft = _draftQuestions[index];
    ...
    return LiquidGlassContainer(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: AppSpacing.x12),
          Text('نوع السؤال', style: AppTextStyles.caption),
          const SizedBox(height: AppSpacing.x8),
          Row(
            children: [
              _typeButton(draft, QuestionType.poll, 'تصويت', Icons.poll_outlined),
              const SizedBox(width: 6),
              _typeButton(draft, QuestionType.quiz, 'اختبار', Icons.school_outlined),
              const SizedBox(width: 6),
              _typeButton(draft, QuestionType.opinion, 'رأي', Icons.psychology_outlined),
              const SizedBox(width: 6),
              _typeButton(draft, QuestionType.discussion, 'نقاش', Icons.forum_outlined),
            ],
          ),
          ...
          _draftOptionField(
            draft: draft,
            label: 'الخيار الأول',
            controller: draft.option1Controller,
          ),
          const SizedBox(height: AppSpacing.x10),
          _draftOptionField(
            draft: draft,
            label: 'الخيار الثاني',
            controller: draft.option2Controller,
          ),
          const SizedBox(height: AppSpacing.x10),
          _draftOptionField(
            draft: draft,
            label: 'الخيار الثالث',
            controller: draft.option3Controller,
          ),
          if (draft.type == QuestionType.quiz) ...[
            const SizedBox(height: AppSpacing.x16),
            Text('الإجابة الصحيحة', style: AppTextStyles.titleMedium),
            const SizedBox(height: AppSpacing.x10),
            _draftCorrectChoice(draft: draft, index: 0),
            const SizedBox(height: AppSpacing.x8),
            _draftCorrectChoice(draft: draft, index: 1),
            const SizedBox(height: AppSpacing.x8),
            _draftCorrectChoice(draft: draft, index: 2),
          ],
        ],
      ),
    );
  ...
  Widget _draftOptionField({
    ...
    return TextFormField(
      ...
      maxLength: 120,
      style: AppTextStyles.bodyLarge,
      decoration: _decoration(label: label, hint: 'اكتب الإجابة...'),
      validator: (value) {
  ...
  Widget _draftCorrectChoice({
    ...
    final selected = draft.correctOptionIndex == index;
  ...
  Widget _buildHashtagSection() {
    return LiquidGlassContainer(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          Text('الهاشتاقات', style: AppTextStyles.titleMedium),
          const SizedBox(height: AppSpacing.x4),
          Text('اختياري — حتى 5 هاشتاقات للمجموعة.', style: AppTextStyles.caption),
          const SizedBox(height: AppSpacing.x12),
          Row(
            children: [
              Expanded(
  ...
  Widget build(BuildContext context) {
    final category = AppCategories.byId(_selectedCategoryId);
      ...
      body: Stack(
        children: [
          LiquidBackground(
            ...
            secondaryOrbColor: category.color.withOpacity(0.4),
          ),
          SafeArea(
            child: Form(
              key: _formKey,
                  ...
                  children: [
                    ...
                    LiquidGlassContainer(
                      child: DropdownButtonFormField<int>(
                        ...
                        items: AppCategories.all.map((item) {
                    ...
                    ...List.generate(
                      ...
                      (index) => Padding(
                        ...
                        child: _questionEditor(index: index),
                      ),
                    ),
                    if (_draftQuestions.length < maxQuestions)
                      OutlinedButton.icon(
                        onPressed: _publishing ? null : _addQuestion,
                        icon: const Icon(Icons.add_rounded),
                        label: const Text('إضافة سؤال آخر'),
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        ...
                        child: _publishing
                            ...
                            : Text('نشر المجموعة (${_draftQuestions.length})'),
                      ),
                    ),
